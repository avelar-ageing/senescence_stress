# 12_pathway_divergence_meta_conditions.R
#
# Which Hallmark pathways differ MOST between the 5 meta-analysis conditions
# themselves (CICQ/SSCQ/RS/SIPS/OIS vs Proliferating) -- i.e. pathways that
# clearly separate one arrest flavor from another, not just "arrest vs
# proliferating" in general. mTOR (the earlier example) is just one instance
# of this; this ranks ALL 50 pathways by that criterion, unbiased.
#
# Divergence = range of Cohen's d across the 5 conditions (max - min) --
# a purely data-driven, topic-blind ranking, same principle as the earlier
# recurrence-based selection for the cross-analysis heatmap, just measuring
# "differs between conditions" instead of "significant somewhere".
...
meta <- df[df$analysis == "meta_analysis" & df$status == "OK", ]
meta$group <- factor(meta$group, levels = c("CICQ", "SSCQ", "RS", "SIPS", "OIS"))
meta$pathway_short <- gsub("^HALLMARK_", "", meta$pathway)

mat_d <- meta %>% select(pathway_short, group, cohens_d_yugene) %>%
  pivot_wider(names_from = group, values_from = cohens_d_yugene) %>% as.data.frame()
rownames(mat_d) <- mat_d$pathway_short; mat_d$pathway_short <- NULL
mat_d <- as.matrix(mat_d)

mat_p <- meta %>% select(pathway_short, group, p_adj_yugene) %>%
  pivot_wider(names_from = group, values_from = p_adj_yugene) %>% as.data.frame()
