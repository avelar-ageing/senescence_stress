...
#
# Logs: rerun_outputs/logs/<script>.log. The driver stops at the first failing script, or
...
export TAGE_PYTHON=$PY TAGE_MODELS_DIR=$MODELS
...
while IFS=$'\t' read -r stage script args produces network; do
  ...
  log="$RERUN/logs/$(echo "$script" | tr '/' '_').log"
